...
# ###############################################################################
# #   This was very hastfully put together and deserves to be archived        # # 
# # does a good job of logging processes and gathering system info, probably  # #
...
function ModifyDB # Unleash S.E.A.L Team Ricks 
{
    ...
    ##### so much had to be changed to support these old ass Warfighter Systems. Ancient PS version
    ...
    $Global:DB.InteractiveLogons += (@{Users = (($Global:DB.InteractiveLogons).($Global:DB.InteractiveLogons.keys))})
    # ($i.InteractiveLogons).($i.InteractiveLogons.Keys)
    #  Assemble Installed Software Inventory Per System
    #$Global:DB.BaselineInstalledArrayOffset = $Global:DB.BaselineInstalled.Count
    
    $Global:DB.BaselineInstalledSoftware = Import-Clixml C:\Scripts\baselineinstalled.clixml
    ...
    #$Global:DB.ProcessLog += Import-Clixml C:\Scripts\processes.clixml 
...
{
    $destFileName = $UploadSharePath + $env:COMPUTERNAME + "_Cronenberged" + ".clixml"
    ...
    while ($(
        ...
        "I FINISHED LOADING PROCESSES" | Out-Host
            ...
            else
            {
                ...
                $global:DB.ProcessLog += Import-Clixml -Path C:\Scripts\processes.clixml 
                $global:processDB += New-ProcessLogEntry
                $global:DB.ProcessLog = $global:processDB
...
function Show-ActiveConnections # Gotta Get that Szechuan Sauce!!!!
{
    ...
    $global:ALLcon =  Get-NetworkStatistics -ShowHostnames
    ...
    foreach ($con in $global:EST_LIST_ONLY)
    {
        foreach ($procName in $global:localProcs)
        {
            if ($con.PID -eq $procName.ParentProcessId)
            {
                ...
                $DNSname = ($con.RemoteAddress).ToString() | Out-Default -ErrorAction SilentlyContinue
...
function Get-NetworkStatistics # Birdperson believes he could arrange that
{
    ...
    .PARAMETER State
	    ...
	    Fin_Wait_2   - The local endpoint of the TCP co`nnection is waiting for a connection termination request from the remote endpoint. 
	...
	process{
        ...
        foreach($Computer in $ComputerName) {
            ...
            if($ShowProcessNames){
                Try {
                    $processes = Get-Process -ComputerName $Computer -ErrorAction stop | ? {$_.Name -ne "CcmExec.exe" | select name, id}
                }
                Catch {
...
function Last24HourFileWrites($yesterday)
{
    cd C:\
    $foundMods = @()
    $dirlist = gci | ? {$_.psiscontainer} | select Name |? {($_.name -ne 'Program Files') -and ($_.name -ne 'Program Files (x86)') -and ($_.name -ne 'Windows') -and ($_.name -ne 'Users')}
    foreach ($dir in $dirlist)
    {
        $p = "C:\" + $dir.Name
        $foundmods += gci -Path $p  -Recurse| ? {$_.lastwriteTime -gt $yesterday}
    }
        $foundMods | Export-Clixml C:\Scripts\Last24FileWrites.clixml
}
#///////////////////////////////////////////////////////////////////////////#
...
function Get-Schwifty # Obviously What you're here for..... I'm Mr. Bulldog
{
    ...
    {
       NetStatLog
       if ($true -ne (Test-Path C:\Scripts\baselineInstalled.clixml -IsValid))
       {
